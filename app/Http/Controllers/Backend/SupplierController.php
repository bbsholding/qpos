...
<form action="' . route('backend.admin.suppliers.destroy', $data->id) . '"method="POST" style="display:inline;">
                   ' . csrf_field() . '
                    ' . method_field("DELETE") . '
<button type="submit" ' . ($data->id == 1 ? 'disabled' : '') . ' class="dropdown-item" onclick="return confirm(\'Êtes-vous sûr de vouloir supprimer ce fournisseur ?\')"><i class="fas fa-trash"></i> Supprimer</button>
                  </form>
                    </div>
                  </div>';
                })
